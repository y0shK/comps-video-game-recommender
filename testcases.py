...
"""
get_pca_fit
Given a query game, obtain fitted PCA for it
Arguments:
...
Returns:
    pca_embed (word embedding), or -1 in case of invalid information
"""
...
def run_testcase(query, rec, model, clf, gamespot_key, giantbomb_key, headers, session):
    ...
    # check to see if pca is actual pca model, or handle invalid input
    if fit_pca == -1:
        print(query, "not enough info to recommend")
        return 0
    
    # obtain rec name from dictionary key
